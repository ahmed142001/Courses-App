...
namespace CourseApp1.Areas.Admin.Controllers
{
    ...
    public class Roadmap_coursesController : Controller
    {
        private courses_dbEntities db = new courses_dbEntities();

        // GET: Admin/Roadmap_courses
        ...
        public ActionResult Create([Bind(Include = "Id,Name,Link,Order,RoadmapId")] Roadmap_courses roadmap_courses,int? id)
        {
            if(id != null)
            {
                roadmap_courses.RoadmapId = (int)id;
            }
            if (ModelState.IsValid)
            {
                db.Roadmap_courses.Add(roadmap_courses);
                db.SaveChanges();
                return RedirectToAction("IndexById", new { id = roadmap_courses.RoadmapId });
            }
